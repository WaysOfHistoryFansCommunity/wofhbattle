import { debugCollector } from './DebugCollector.js'; 

let Module = typeof Module != "undefined" ? Module : {};
Module = {
    canvas: document.getElementById("game"),
    locateFile: function (file) {
        if (file === "engine.wasm") return window.ge.scriptPrefix + "js/engine.wasm";
        if (file === "GeWebLauncher.wasm") return window.ge.scriptPrefix + "js/platform/GeWebLauncher.wasm";
        return file;
    },
    arguments: [],
};
...
window.ge.fonts = [];
console.assert(window.ge.namedConst.extPlat);
Module["arguments"].push(window.ge.namedConst.extPlat);
console.assert(window.ge.namedConst.extAcc);
Module["arguments"].push(window.ge.namedConst.extAcc);
console.assert(window.ge.namedConst.extSig);
...
let INITIAL_MEMORY = Module["INITIAL_MEMORY"] || 503316480;
let wasmTable;
let __AT_PRE_RUN__ = [];
let __AT_INIT__ = [];
let __AT_MAIN__ = [];
...
function preRun() 
{
    ...
    callRuntimeCallbacks(__AT_PRE_RUN__);
}
function initRuntime() 
{
    runtimeInitialized = true;
    callRuntimeCallbacks(__AT_INIT__);
}
function preMain() {
    callRuntimeCallbacks(__AT_MAIN__);
}
function postRun() {
    if (Module["postRun"]) 
    {
    ...
    callRuntimeCallbacks(__AT_POST_RUN__);
}
function addOnPreRun(cb) {
    __AT_PRE_RUN__.unshift(cb);
}
function addOnInit(cb) {
    __AT_INIT__.unshift(cb);
}
function addOnPostRun(cb) {
    __AT_POST_RUN__.unshift(cb);
}
...
function createWasm() {
    ...
    function receiveInstance(instance, module) 
    {
        ...
        wasmMemory = Module["asm"]["Me"];
        updateGlobalBufferAndViews(wasmMemory.buffer);
        wasmTable = Module["asm"]["Ve"];
        addOnInit(Module["asm"]["Ne"]);
...
var ASM_CONSTS = {
    2479464: ($0, $1, $2, $3, $4) => {
        console.error(
            UTF8ToString($0) + " " + UTF8ToString($1) + ":" + $2 + ":" + UTF8ToString($3) + " with " + UTF8ToString($4)
        );
    },
    2479591: ($0) => {
        if (!$0) {
...
function _Helper_Payment_make(product, value, payloadPtr, payloadSize) {
    var payload = UTF8ToString(payloadPtr, payloadSize);
    window.ge.makePayment(product, value, payload);
}
...
function _Helper_WebSock_connect(handle, hostNamePtr, hostNameSize) {
    assert(handle < window.ge.webSockHelpers.length);
    var wsHelper = window.ge.webSockHelpers[handle];
    var hostName = UTF8ToString(hostNamePtr, hostNameSize);
    return wsHelper.connect(hostName);
}
function _Helper_WebSock_createHandle() {
    if (window.ge.webSockHelpers === undefined) window.ge.webSockHelpers = [];
    for (var i = 0; i < window.ge.webSockHelpers.length; ++i) {
        if (window.ge.webSockHelpers[i] === null) {
            window.ge.webSockHelpers[i] = new WebSockHelper();
            return i;
        }
    }
    var index = window.ge.webSockHelpers.length;
    window.ge.webSockHelpers[index] = new WebSockHelper();
    return index;
}
function _Helper_WebSock_freeHandle(handle) {
    assert(window.ge.webSockHelpers !== undefined);
    assert(window.ge.webSockHelpers[handle] != null);
    window.ge.webSockHelpers[handle].disconnect();
    window.ge.webSockHelpers[handle] = null;
}
function _Helper_WebSock_isConnected(handle) {
    assert(handle < window.ge.webSockHelpers.length);
    var wsHelper = window.ge.webSockHelpers[handle];
    return wsHelper.isConnected();
}
function _Helper_WebSock_receiveMessage(handle, lengthPtr) {
    assert(handle < window.ge.webSockHelpers.length);
    var wsHelper = window.ge.webSockHelpers[handle];
    return wsHelper.receiveMessage(lengthPtr);
}
function _Helper_WebSock_sendMessage(handle, dataPtr, dataSize) {
    assert(handle < window.ge.webSockHelpers.length);
    var wsHelper = window.ge.webSockHelpers[handle];
    wsHelper.sendMessage(dataPtr, dataSize);
}
function _Helper_engineReady() {
    Module.canvas.removeAttribute("hidden");
    var prestart = document.getElementById("prestartScreen");
    prestart.parentNode.removeChild(prestart);
    if (window.ge.friendsPanel) window.ge.friendsPanel.makeVisible();
}
function _Helper_friends_check() {
    return true;
}
function _Helper_friends_count() {
    return 1;
}
function _Helper_friends_get(idx, resultPtr, resultSize) {
    if (idx == 0) {
        stringToUTF8("test".toString(), resultPtr, resultSize);
        return true;
    }
    return false;
}
function _Helper_friends_request() {
    if (window.ge.requestFriends) window.ge.requestFriends();
}
function _Helper_getGlobalVersion() {
    return window.ge.version;
}
function _Helper_getLanguage(pointer, size) {
    var lang = window.ge.namedConst["lang"] === undefined ? "" : window.ge.namedConst["lang"];
    if (!lang) lang = window.navigator.language.split("-")[0];
    console.assert(lang);
    stringToUTF8(lang, pointer, size);
}
...
function _Helper_openLink(paramPtr, paramSize) {
    var url = UTF8ToString(paramPtr, paramSize);
    window.open(url);
}
function _Helper_showMessage(titlePtr, titleSize, messagePtr, messageSize) {
    var title = UTF8ToString(titlePtr, titleSize);
    var message = UTF8ToString(messagePtr, messageSize);
    window.alert(title + ". " + message);
}
function _Helper_switchToFullScreen() {
    if (Module.canvas.requestFullscreen) Module.canvas.requestFullscreen();
    else if (Module.canvas.webkitRequestFullscreen) Module.canvas.webkitRequestFullscreen();
    else if (Module.canvas.msRequestFullscreen) Module.canvas.msRequestFullscreen();
    else console.assert(false);
}
function _Helper_switchToWindowed() {
...
function _checkTextureParsing(taskId) {
    var parsingTask = parsingTextures[taskId];
    if (parsingTask === undefined) return -1;
    if (parsingTask === null) return -1;
    return parsingTask.state;
}
...
let __emscripten_webgl_power_preferences = ["default", "low-power", "high-performance"];
function maybeCStringToJsString(cString) {
    return cString > 2 ? UTF8ToString(cString) : cString;
}
let specialHTMLTargets = [0, typeof document != "undefined" ? document : 0, typeof window != "undefined" ? window : 0];
...
function _genParseTask() 
{
    if (typeof parsingTextures === "undefined") parsingTextures = [];
    idx = parsingTextures.findIndex(function (elem, index, array) 
    {
        return elem === undefined || elem === null;
    });
    if (idx === -1) idx = parsingTextures.push(undefined) - 1;
    return idx;
}
function _getTextureRgba(taskId, bytesPerPixel, ptr) 
{
    var parsingTask = parsingTextures[taskId];
    assert(parsingTask.state === 1);
    const imageData = parsingTask.image.ctx2d.getImageData(0, 0, parsingTask.image.width, parsingTask.image.height);
    const size = parsingTask.image.width * parsingTask.image.height * bytesPerPixel;
    const destination = new Uint8ClampedArray(Module.HEAPU8.buffer, ptr, size);
    switch (bytesPerPixel) {
        case 1:
            const red = imageData.data.filter(function (element, index, array) {
                return index % 4 === 0;
            });
            destination.set(red);
            break;
        case 3:
            const rgb = imageData.data.filter(function (element, index, array) {
                return (index + 1) % 4 !== 0;
            });
            destination.set(rgb);
            break;
        case 4:
            destination.set(imageData.data);
            break;
        default:
            console.error("Invalid bytes per pixel.");
            break;
    }
    parsingTask.image.ctx2d = null;
}
function _getTextureSizeX(taskId) 
{
    var parsingTask = parsingTextures[taskId];
    assert(parsingTask.state === 1);
    return parsingTask.image.width;
}
function _getTextureSizeY(taskId) 
{
    var parsingTask = parsingTextures[taskId];
    assert(parsingTask.state === 1);
    return parsingTask.image.height;
}
...
function _isWebgl1Supported() {
    var canvas = document.createElement("canvas");
    var support = false;
    var supportExp = false;
    try {
        support = !!canvas.getContext("webgl");
    } catch (e) {}
    try {
        supportExp = !!canvas.getContext("experimental-webgl");
    } catch (e) {}
    canvas = null;
    return support || supportExp;
}
function _isWebgl2Supported() {
    ...
    var supportExp = false;
    try {
        support = !!canvas.getContext("webgl2");
    } catch (e) {}
    try {
        supportExp = !!canvas.getContext("experimental-webgl2");
    } catch (e) {}
    canvas = null;
    return support || supportExp;
}
function _manager_init() {
    this.Manager = {
        handle: null,
        init: function () {
            ...
            addEvent(window, "mouseout", function () {
                Manager.im_mouseLeave();
            });
            addEvent(window, "mouseover", function () {
                Manager.im_mouseReturn();
            });
            this.im_update = Module.cwrap("platformWork", null, []);
            this.im_mouseLeave = Module.cwrap("mouseLeave", null, []);
            this.im_mouseReturn = Module.cwrap("mouseReturn", null, []);
        ...
        work: function () {
            Manager.im_update();
            window.requestAnimationFrame(Manager.work);
        },
    };
    Manager.init();
...
function _r_checkTextureParsing(taskId) {
    var parsingTask = window.ge.r_parsingTextures[taskId];
    if (parsingTask === undefined) return -1;
    if (parsingTask === null) return -1;
    return parsingTask.state;
}
function _r_getTextureHeight(taskId) {
    var parsingTask = window.ge.r_parsingTextures[taskId];
    assert(parsingTask.state == 1);
    return parsingTask.image.height;
}
function _r_getTextureRgba(taskId, ptr, bytesPerPixel) {
    var parsingTask = window.ge.r_parsingTextures[taskId];
    assert(parsingTask.state == 1);
    const imageData = parsingTask.image.ctx2d.getImageData(0, 0, parsingTask.image.width, parsingTask.image.height);
    const size = parsingTask.image.width * parsingTask.image.height * bytesPerPixel;
    ...
    switch (bytesPerPixel) {
        case 1:
            ...
            break;
        case 2:
            const rg = imageData.data.filter(function (element, index, array) {
        ...
        case 4:
            ...
            break;
        default:
            throw "Invalid bytes per pixel.";
    }
    parsingTask.image.ctx2d = null;
}
function _r_getTextureWidth(taskId) {
    var parsingTask = window.ge.r_parsingTextures[taskId];
    assert(parsingTask.state == 1);
    return parsingTask.image.width;
}
function _r_removeParseTask(taskId) {
    var parsingTask = window.ge.r_parsingTextures[taskId];
    parsingTask.image = null;
    window.ge.r_parsingTextures[taskId] = null;
}
function _r_startParseTexture(taskId, data, sz) {
    window.ge.r_parsingTextures[taskId] = {};
    var parsingTask = window.ge.r_parsingTextures[taskId];
    parsingTask.state = 0;
    parsingTask.image = new Image();
    var u8a = Module.HEAPU8.buffer.slice(data, data + sz);
    ...
    var resizeImage = function (image) {
        ...
        canvas.ctx2d.drawImage(image, 0, 0, canvas.width, canvas.height);
        return canvas;
    };
    parsingTask.image.onload = function () {
        parsingTask.image = resizeImage(parsingTask.image);
        parsingTask.state = 1;
    };
    parsingTask.image.src = URL.createObjectURL(blob);
}
function _startParseTexture(taskId, data, sz) {
    parsingTextures[taskId] = {};
    var parsingTask = parsingTextures[taskId];
    parsingTask.state = 0;
    parsingTask.image = new Image();
    var arrayBuffer = new ArrayBuffer(sz);
    var arrayBufferU8 = new Uint8Array(arrayBuffer);
    arrayBufferU8.set(Module.HEAPU8.slice(data, data + sz));
    var blob = new Blob([arrayBufferU8]);
    parsingTask.image.onload = function () {
        var imgCanvas = document.createElement("canvas");
        if (parsingTask.image == null) return;
        imgCanvas.width = parsingTask.image.width;
        imgCanvas.height = parsingTask.image.height;
        imgCanvas.ctx2d = imgCanvas.getContext("2d");
        imgCanvas.ctx2d.drawImage(parsingTask.image, 0, 0, imgCanvas.width, imgCanvas.height);
        parsingTask.image = imgCanvas;
        console.assert(parsingTask.image != null);
        parsingTask.state = 1;
    };
    parsingTask.image.src = URL.createObjectURL(blob);
}
    ...
    var EXPANSION_RULES_2 = {
        ...
        "%j": function (date) {
            return leadingNulls(
                date.tm_mday +
                    __arraySum(
                        __isLeapYear(date.tm_year + 1900) ? MONTH_DAYS_LEAP : MONTH_DAYS_REGULAR,
                        date.tm_mon - 1
                    ),
                3
            );
...
function _webglBlendEquationSeparateiOES(buf, modeRGB, modeAlpha) 
{
    console.assert(window.geDbiExt != null);
    window.geDbiExt.blendEquationSeparateiOES(buf, modeRGB, modeAlpha);
}
function _webglBlendEquationiOES(buf, mode) 
{
    console.assert(window.geDbiExt != null);
    window.geDbiExt.blendEquationiOES(buf, mode);
}
function _webglBlendFuncSeparateiOES(buf, srcRGB, dstRGB, srcAlpha, dstAlpha) 
{
    console.assert(window.geDbiExt != null);
    window.geDbiExt.blendFuncSeparateiOES(buf, srcRGB, dstRGB, srcAlpha, dstAlpha);
}
function _webglBlendFunciOES(buf, src, dst) 
{
    console.assert(window.geDbiExt != null);
    window.geDbiExt.blendFunciOES(buf, src, dst);
}
function _webglColorMaskiOES(buf, maskR, maskG, maskB, maskA)
{
    console.assert(window.geDbiExt != null);
    window.geDbiExt.colorMaskiOES(buf, maskR, maskG, maskB, maskA);
}
function _webglEnableDrawBuffersIndexedExt()
{
    if (window.geDbiExt == null) {
        window.geDbiExt = GLctx.getExtension("OES_draw_buffers_indexed");
        if (window.geDbiExt == null) {
            console.log("OES_draw_buffers_indexed is not available");
        }
    }
}
function _webglIsDrawBuffersIndexedAvailable() 
{
    return window.geDbiExt != null;
}
...
let miniTempWebGLFloatBuffersStorage = new Float32Array(288);
for (var i = 0; i < 288; ++i) 
{
    miniTempWebGLFloatBuffers[i] = miniTempWebGLFloatBuffersStorage.subarray(0, i + 1);
}
let __miniTempWebGLIntBuffersStorage = new Int32Array(288);
for (var i = 0; i < 288; ++i) 
{
...
let asmLibraryArg = 
{
    Ha: _Helper_Payment_make,
    Eb: _Helper_Persistent_get,
    Fb: _Helper_Persistent_getSize,
    C: _Helper_Persistent_store,
    L: _Helper_Requester_getData,
    S: _Helper_Requester_getProgress,
    R: _Helper_Requester_getStatus,
    H: _Helper_Requester_isDone,
    M: _Helper_Requester_isSuccess,
    la: _Helper_Requester_makeRequest,
    Ia: _Helper_Requester_releaseRequester,
    Jb: _Helper_WebSock_connect,
    Hb: _Helper_WebSock_createHandle,
    Gb: _Helper_WebSock_freeHandle,
    Lb: _Helper_WebSock_isConnected,
    Kb: _Helper_WebSock_receiveMessage,
    Mb: _Helper_WebSock_sendMessage,
    Ub: _Helper_engineReady,
    Xb: _Helper_friends_check,
    Wb: _Helper_friends_count,
    Db: _Helper_friends_get,
    Yb: _Helper_friends_request,
    pc: _Helper_getGlobalVersion,
    Nb: _Helper_getLanguage,
    Ob: _Helper_getNamedConst,
    Zb: _Helper_openLink,
    _b: _Helper_showMessage,
    hc: _Helper_switchToFullScreen,
    gc: _Helper_switchToWindowed,
    g: ___assert_fail,
    xc: ___syscall_dup3,
    oa: ___syscall_fcntl64,
    Bc: ___syscall_ioctl,
    vc: ___syscall_lstat64,
    Qa: ___syscall_openat,
    rc: ___syscall_renameat,
    sc: ___syscall_rmdir,
    wc: ___syscall_stat64,
    Pa: ___syscall_unlinkat,
    Cc: __emscripten_get_now_is_monotonic,
    oc: __emscripten_throw_longjmp,
    Dc: __gmtime_js,
    Ec: __localtime_js,
    Fc: __mktime_js,
    Gc: __tzset_js,
    ...
    dd: _alSourcePause,
    gd: _alSourcePlay,
    Za: _alSourceQueueBuffers,
    Ya: _alSourceStop,
    U: _alSourceUnqueueBuffers,
    _: _alSourcef,
    I: _alSourcei,
    Rc: _alcCaptureCloseDevice,
    Sc: _alcCaptureOpenDevice,
    Nc: _alcCaptureSamples,
    Qc: _alcCaptureStart,
    Oc: _alcCaptureStop,
    ra: _alcCloseDevice,
    $a: _alcCreateContext,
    qa: _alcDestroyContext,
    Vc: _alcGetContextsDevice,
    Wc: _alcGetCurrentContext,
    Uc: _alcGetEnumValue,
    Ta: _alcGetError,
    Tc: _alcGetIntegerv,
    aa: _alcGetString,
    V: _alcIsExtensionPresent,
    $: _alcMakeContextCurrent,
    sa: _alcOpenDevice,
    Yc: _alcProcessContext,
    Xc: _alcSuspendContext,
    Rb: _checkTextureParsing,
    Mc: _emscripten_alcDevicePauseSOFT,
    Lc: _emscripten_alcDeviceResumeSOFT,
    Kc: _emscripten_alcGetStringiSOFT,
    Jc: _emscripten_alcResetDeviceSOFT,
    Ab: _emscripten_asm_const_int,
    Ic: _emscripten_console_error,
    Z: _emscripten_date_now,
    od: _emscripten_get_device_pixel_ratio,
    pa: _emscripten_get_now,
    Hc: _emscripten_memcpy_big,
    qc: _emscripten_resize_heap,
    rd: _emscripten_webgl_commit_frame,
    td: _emscripten_webgl_create_context,
    p: _emscripten_webgl_enable_extension,
    q: _emscripten_webgl_get_current_context,
    ab: _emscripten_webgl_make_context_current,
    tc: _environ_get,
    ...
    n: _glActiveTexture,
    te: _glAttachShader,
    Sd: _glBeginTransformFeedback,
    Ad: _glBindAttribLocation,
    b: _glBindBuffer,
    Qd: _glBindBufferBase,
    N: _glBindBufferRange,
    h: _glBindFramebuffer,
    a: _glBindTexture,
    d: _glBindVertexArray,
    ha: _glBlendColor,
    B: _glBlendEquation,
    Ke: _glBlendEquationSeparate,
    O: _glBlendFunc,
    wb: _glBlendFuncSeparate,
    eb: _glBlitFramebuffer,
    y: _glBufferData,
    za: _glBufferSubData,
    ...
    Je: _glColorMask,
    pe: _glCompileShader,
    gb: _glCompressedTexSubImage2D,
    nd: _glCopyBufferSubData,
    Ce: _glCreateProgram,
    re: _glCreateShader,
    Ea: _glCullFace,
    D: _glDeleteBuffers,
    ...
    ue: _glDeleteShader,
    G: _glDeleteTextures,
    fa: _glDeleteVertexArrays,
    ia: _glDepthFunc,
    xb: _glDepthMask,
    ve: _glDetachShader,
    z: _glDisable,
    jb: _glDrawArrays,
    Cd: _glDrawArraysInstanced,
    Hd: _glDrawBuffers,
    Aa: _glDrawElements,
    Bd: _glDrawElementsInstanced,
    v: _glEnable,
    ea: _glEnableVertexAttribArray,
    Rd: _glEndTransformFeedback,
    o: _glFramebufferTexture2D,
    E: _glGenBuffers,
    A: _glGenFramebuffers,
    w: _glGenTextures,
    xa: _glGenVertexArrays,
    wa: _glGenerateMipmap,
    we: _glGetAttachedShaders,
    ya: _glGetBufferParameteriv,
    va: _glGetBufferSubData,
    f: _glGetError,
    da: _glGetFramebufferAttachmentParameteriv,
    e: _glGetIntegerv,
    ne: _glGetProgramInfoLog,
    ga: _glGetProgramiv,
    oe: _glGetShaderInfoLog,
    pb: _glGetShaderiv,
    i: _glGetString,
    Ed: _glGetTexParameteriv,
    ye: _glGetUniformBlockIndex,
    rb: _glGetUniformLocation,
    Fa: _glIsFramebuffer,
    r: _glIsProgram,
    K: _glIsShader,
    Q: _glIsTexture,
    ...
    Ee: _glStencilMask,
    sb: _glStencilMaskSeparate,
    Fe: _glStencilOp,
    tb: _glStencilOpSeparate,
    t: _glTexImage2D,
    db: _glTexImage3D,
    W: _glTexParameterf,
    Fd: _glTexParameterfv,
    j: _glTexParameteri,
    yd: _glTexStorage2D,
    fb: _glTexSubImage2D,
    xd: _glTexSubImage3D,
    Be: _glTransformFeedbackVaryings,
    me: _glUniform1f,
    je: _glUniform1fv,
    qb: _glUniform1i,
    ie: _glUniform1iv,
    ...
    ae: _glUniform4ui,
    Wd: _glUniform4uiv,
    xe: _glUniformBlockBinding,
    Vd: _glUniformMatrix2fv,
    Ud: _glUniformMatrix3fv,
    kb: _glUniformMatrix3x4fv,
    Ba: _glUniformMatrix4fv,
    lb: _glUniformMatrix4x3fv,
    P: _glUseProgram,
    hb: _glVertexAttribDivisor,
    ca: _glVertexAttribI4i,
    ba: _glVertexAttribI4ui,
    wd: _glVertexAttribIPointer,
    cb: _glVertexAttribPointer,
    Ca: _glViewport,
    bb: _glewGetErrorString,
    c: _glewGetExtension,
    ta: _glewInit,
    ja: _glewIsSupported,
    ed: _glfwCreateCursor,
    zb: _glfwCreateWindow,
    Vb: _glfwDestroyWindow,
    Pc: _glfwGetClipboardString,
    Sa: _glfwGetCursorPos,
    qd: _glfwGetMonitorPhysicalSize,
    _a: _glfwGetPrimaryMonitor,
    pd: _glfwGetVideoMode,
    Oa: _glfwGetWindowPos,
    yc: _glfwGetWindowSize,
    kc: _glfwIconifyWindow,
    Ib: _glfwInit,
    T: _glfwMakeContextCurrent,
    jc: _glfwMaximizeWindow,
    fc: _glfwPollEvents,
    Pd: _glfwSetCharCallback,
    Ra: _glfwSetClipboardString,
    ma: _glfwSetCursor,
    Dd: _glfwSetCursorEnterCallback,
    ce: _glfwSetCursorPosCallback,
    ka: _glfwSetErrorCallback,
    ke: _glfwSetKeyCallback,
    se: _glfwSetMouseButtonCallback,
    Td: _glfwSetScrollCallback,
    sd: _glfwSetTime,
    Id: _glfwSetWindowCloseCallback,
    zd: _glfwSetWindowFocusCallback,
    Ka: _glfwSetWindowPos,
    Ae: _glfwSetWindowSizeCallback,
    Le: _glfwSetWindowSizeLimits,
    ic: _glfwSetWindowTitle,
    vd: _glfwSwapBuffers,
    ud: _glfwSwapInterval,
    Ja: _glfwTerminate,
    F: _glfwWindowHint,
    Na: invoke,
    Y: invoke_iiiii,
    ...
    $c: invoke_vii,
    Ma: invoke_viii,
    Bb: _isWebgl1Supported,
    X: _isWebgl2Supported,
    He: _manager_init,
    dc: _r_checkTextureParsing,
    bc: _r_getTextureHeight,
    ac: _r_getTextureRgba,
    cc: _r_getTextureWidth,
    $b: _r_removeParseTask,
    ec: _r_startParseTexture,
    Sb: _startParseTexture,
    _c: _strftime,
    nc: _strftime_l,
    Zc: _system,
    Jd: _webglBlendEquationSeparateiOES,
    Kd: _webglBlendEquationiOES,
    Ld: _webglBlendFuncSeparateiOES,
    Md: _webglBlendFunciOES,
    Nd: _webglColorMaskiOES,
    ib: _webglEnableDrawBuffersIndexedExt,
    Od: _webglIsDrawBuffersIndexedAvailable,
};
...
var ___wasm_call_ctors = (Module["___wasm_call_ctors"] = function () {
    return (___wasm_call_ctors = Module["___wasm_call_ctors"] = Module["asm"]["Ne"]).apply(null, arguments);
});
var _platformWork = (Module["_platformWork"] = function () {
    return (_platformWork = Module["_platformWork"] = Module["asm"]["Oe"]).apply(null, arguments);
});
var _main = (Module["_main"] = function () {
    return (_main = Module["_main"] = Module["asm"]["Pe"]).apply(null, arguments);
});
var _free = (Module["_free"] = function () {
    return (_free = Module["_free"] = Module["asm"]["Qe"]).apply(null, arguments);
});
var _malloc = (Module["_malloc"] = function () {
    return (_malloc = Module["_malloc"] = Module["asm"]["Re"]).apply(null, arguments);
});
var _mouseLeave = (Module["_mouseLeave"] = function () {
    return (_mouseLeave = Module["_mouseLeave"] = Module["asm"]["Se"]).apply(null, arguments);
});
var _mouseReturn = (Module["_mouseReturn"] = function () {
    return (_mouseReturn = Module["_mouseReturn"] = Module["asm"]["Te"]).apply(null, arguments);
});
var ___errno_location = (Module["___errno_location"] = function () {
    return (___errno_location = Module["___errno_location"] = Module["asm"]["Ue"]).apply(null, arguments);
});
var ___trap = (Module["___trap"] = function () {
    return (___trap = Module["___trap"] = Module["asm"]["We"]).apply(null, arguments);
});
var _setThrew = (Module["_setThrew"] = function () {
    return (_setThrew = Module["_setThrew"] = Module["asm"]["Xe"]).apply(null, arguments);
});
var stackSave = (Module["stackSave"] = function () {
    return (stackSave = Module["stackSave"] = Module["asm"]["Ye"]).apply(null, arguments);
});
var stackRestore = (Module["stackRestore"] = function () {
    return (stackRestore = Module["stackRestore"] = Module["asm"]["Ze"]).apply(null, arguments);
});
var stackAlloc = (Module["stackAlloc"] = function () {
    return (stackAlloc = Module["stackAlloc"] = Module["asm"]["_e"]).apply(null, arguments);
});
