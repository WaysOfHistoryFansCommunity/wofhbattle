...
const domain = 'ru69.waysofhistory.com';


function createSimulationUrl(simulation) 
{
    ...
    params.append('type', 'simulation');
    params.append('simulation', JSON.stringify(simulation));
    params.append('replay', Math.floor(Math.random() * 999999999));
...
document.querySelector('.simbattle-run').addEventListener('click', () => 
{
    ...
    if(Object.keys(simArmyAttack).length && Object.keys(simArmyDefense).length)
    {
        ...
        const url = createSimulationUrl(simulation);
...
document.querySelector('.simbattle-run-lastreplay').addEventListener('click', () => 
{
    const windowTitle = `Last Replay Window ${windowCount}`;
    const windowId = `window_${windowCount}`;
    windowCount++;
    
    const url = createReplayUrl('replays/last.rep');
